        ...
        try:
            ...
            for command,callback in self._commands.items():
                if text.startswith(command):
                    stripped = text[len(command):].strip()
                    callback(stripped,send_message_cb=self._send_message, mongo_client=self._mongo_client)
                    return
            if text.startswith("/"):
            ...
            self._commands[None](text,send_message_cb=self._send_message, mongo_client=self._mongo_client)
        except e:
            #FIXME: is catch-all catcher inappropriate here?
            self._send_message(f"```{}```",parse_mode="Markdown")
            raise e
