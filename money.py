...
def _get_coll(mongo_pass, is_return_client=False):
    ...
    return (coll, client) if is_return_client else coll
...
@click.pass_context
def money(ctx, debug, mongo_pass):
    if debug:
...
@money.command()
@click.argument("money_amount_expr")
@click.pass_context
def add(ctx, money_amount_expr):
    mongo_pass = ctx.obj["mongo_pass"]
    _, mongo_client = _get_coll(mongo_pass, is_return_client=True)
    # def add_money(text, send_message_cb=None, mongo_client=None):
    add_money(
        money_amount_expr, send_message_cb=logging.warning, mongo_client=mongo_client
    )
...
@click.option(
    "-m", "--mode", type=click.Choice(["daily", "monthly", "weekly"]), default="daily"
)
@click.option(
    "--monthly-regular-payments-file-name",
    type=click.Path(),
    default=".monthly_regular_payments.json",
)
...
def show(
    ...
    day,
    mode,
    monthly_regular_payments_file_name,
    monthly_channel_webhook,
    send_slack_message,
    weekly_channel_webhook,
    show_top_expenses,
    logger=None,
):
    mongo_pass = ctx.obj["mongo_pass"]
    ...
    if mode == "daily":
        money_df = pd.DataFrame(
            coll.find(
                {
                    "$and": [
                        {"date": {"$gte": datetime(day.year, day.month, day.day)}},
                        {
                            "date": {
                                "$lt": datetime(day.year, day.month, day.day)
                                + timedelta(days=1)
                            }
                        },
                    ]
        ...
        money_df = pd.DataFrame(
            coll.find(
                {
                    "$and": [
                        {"date": {"$gte": datetime(day.year, day.month, 1)}},
                        {"date": {"$lt": datetime(*next_month, 1) + timedelta(days=1)}},
                    ]
                },
                sort=[("date", pymongo.DESCENDING)],
            )
        ...
        money_df = pd.DataFrame(
            coll.find(
                {
                    "$and": [
                        {"date": {"$gte": last_week_start_inc}},
                        {"date": {"$lt": last_week_end_exc}},
                    ]
                },
                sort=[("date", pymongo.DESCENDING)],
            )
